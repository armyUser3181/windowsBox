...
**제안**: `id`는 사람이 읽는 이름(로깅/디버깅용)으로, `symbol`은 실제 Map 키로 용도를 나눠서 통일하세요. 지금처럼 두 개를 만들어놓고 하나만 쓰는 상태로 두면, 나중에 `push`/`remove`/`renew`를 다 구현한 뒤에 "어? symbol은 왜 있
지?" 하고 다시 고민하게 됩니다.
...
구현에 들어가기 전에, 아래 다섯 가지만 먼저 결정해두시면 이후 작업이 훨씬 더 매끄러울 것 같습니다.
...
이 다섯 개는 전부 "코드를 얼마나 쓰느냐"의 문제가 아니라 **"이름과 타입을 미리 통일해두는" 설계 마무리 작업**이라, 지금처럼 아직 로직을 채우기 전 단계에서 정리하는 게 나중에 다시 뜯어고치는 것보다 훨씬 쌉습니다.
